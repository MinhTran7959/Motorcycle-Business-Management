...
$(document).on('change', '.idpxk-dropdown', function () {
    ...
    var idkh = selectedOption.data('idkh');
    var idnv = selectedOption.data('idnv');
    var tenkh = selectedOption.data('tenkh');
    var diachikh = selectedOption.data('diachikh');
    var dienthoaikh = selectedOption.data('sodienthoaikh');
    var tennv = selectedOption.data('tennv');
    var tongtien = selectedOption.data('tongtien');
    var ngayxuatStr = selectedOption.data('ngayxuat');
    var sopxk = selectedOption.data('sopxk');
    ...
    var ngayxuat = new Date(ngayxuatStr);

    // Định dạng ngày nhập thành "dd/mm/yyyy"
    var formattedNgayxuat = ngayxuat.toLocaleDateString('en-IN', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric'
    }).split('/').reverse().join('-');
    ...
    row.find('.Tongtienxuat').val(formattedTongtien);
    row.find('.Sohd').val(sohd);
    row.find('.Idkh').val(idkh);
    row.find('.Idnv').val(idnv);
    row.find('.Tenkh').val(tenkh);
    row.find('.Diachikh').val(diachikh);
    row.find('.Dienthoaikh').val(dienthoaikh);
    row.find('.Tennv').val(tennv);
    row.find('.Ngayhd').val(formattedNgayhd);
    row.find('.Ngayxuat').val(formattedNgayxuat);
    row.find('.Sopxk').val(sopxk);
...
function DeleteItem(btn) {
    ...
    var gianhapInput = row.find('input[id^="Tongtienxuat-"]');
    gianhapInput.val(0);
...
function formatCurrencyAndTotal(input) {
    ...
    var total = 0;
    var ithanhtienElements = document.querySelectorAll('[id^="Tongtienxuat-"]');
...
function checkDuplicateIdct(selectedIdct, element) {
    ...
    var idctDropdowns = document.getElementsByClassName('idpxk-dropdown');
    ...
    for (var i = 0; i < idctDropdowns.length; i++) {
        ...
        if (currentIdct === selectedIdct && isDuplicate) {
            showToast('Phiếu bán xe đã được chọn trước đó ở một dòng khác. Vui lòng chọn xe khác', true);
